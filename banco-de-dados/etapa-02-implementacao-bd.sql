...
CREATE TABLE Locacao (
 ...
 idPagamento INT NOT NULL UNIQUE, -- relacionamento 1:1
...
SELECT SUM(valorTotal) AS valorArrecadado FROM Pagamento WHERE estado = 'Pago';

SELECT 
    ve.modelo, ve.marca, COUNT(lv.idLocacao) AS totalLocacoes
FROM
    Veiculo AS ve
        JOIN
    LocacaoVeiculo AS lv ON ve.idVeiculo = lv.idVeiculo
GROUP BY ve.idVeiculo
ORDER BY totalLocacoes DESC;

SELECT 
    cli.nome, SUM(pag.valorTotal) AS valorDevido
FROM
    Cliente AS cli
        JOIN
    Locacao AS loc ON cli.idCliente = loc.idCliente
        JOIN
    Pagamento AS pag ON loc.idPagamento = pag.idPagamento
WHERE
    pag.estado = 'Pendente'
GROUP BY cli.nome
